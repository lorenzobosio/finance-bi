...
- **D-28 (SPIKE FINDING — Lorenzo's login, 2026-06-22):** Enable Banking exposes Lorenzo's **personal + joint** accounts but **NOT the investing account** — confirming **A2** (the investment pocket is outside PSD2). Consequences, all already anticipated: (a) `investimento` is detected on the **outgoing leg** — a transfer *from* a cash account whose destination/counterparty is the investing account — since the incoming leg is invisible; (b) the investing account is modeled as a **virtual `is_investment=true` row** (D-22); (c) MVP "total invested" stays **cost basis** from those outgoing transfers (D-23). **Phase 6 market value (refines D-06 / ETF-04):** the bank can't provide holdings, so track them **virtually** — the app periodically asks how many ETF units are held (e.g. of ISIN IE000716YHJ7), valued via a prices API, with a manual override. (Still to confirm via the live `eb:connect` run: real `access.valid_until`, counterparty/IBAN availability on the outgoing transfer, id stability, PEND presence.)
